 ...
 * Usage: Call this from your client components
 */
export const scrapeUrlWithFirecrawl = createServerFn({ method: "POST" })
	.handler(async ({ data }) => {
		const { url, formats } = (data ?? {}) as { url: string; formats?: ('markdown' | 'html')[] };
		...
		try {
			...
			// Extract only the serializable properties and serialize metadata to avoid type issues
	...
	.handler(async ({ data }) => {
		...
		try {
			...
			const extractedInfo = {
				...
				date: extractDate(markdown + html),
